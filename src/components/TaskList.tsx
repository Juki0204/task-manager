...
export default function TaskList({ taskList, user, unreadIds, importantIds, onClick, onContextMenu }: TaskListProps) {
  ...
  return (
    <div className="pb-4 [&::-webkit-scrollbar]:h-1 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-track]:bg-neutral-600 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-neutral-500
      ...
      {taskList.map(task => (
        <Card user={user} key={task.id} task={task} unreadIds={unreadIds} importantIds={importantIds} onClick={onClick} onContextMenu={onContextMenu} />
      ))}
    </div>
  )
}