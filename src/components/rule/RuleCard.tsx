...
import { Newspaper, Repeat, Smile, SquarePen } from "lucide-react";
...
export default function RuleCard({ rule, acknowledgements, users, onClick }: RuleCardProps) {
  ...
  return (
    <div onClick={() => onClick(rule)} className="flex flex-col w-full min-w-200 border-b border-neutral-300 dark:border-neutral-600 text-neutral-700 dark:text-neutral-100 gap-2 p-3 hover:bg-black/4 hover:dark:bg-white/10 hover:cursor-pointer">
      <div className="flex flex-col w-full">
        <div className="flex justify-between">
          <h3 className="flex w-[calc(100%-360px)] gap-1 items-center font-bold tracking-widest">
            {isAcknowledged === "new" && <span className="w-16 text-center grid place-content-center bg-blue-400 px-3 pt-0.25 rounded-full text-sm text-white font-medium">未読</span>}
            {isAcknowledged === "updated" && <span className="w-16 text-center grid place-content-center bg-red-400 px-3 pt-0.25 rounded-full text-sm text-white font-medium">追記</span>}
            <Newspaper className="w-5" />
            <span className="w-full truncate">{rule.title}</span>
          </h3>
      ...
      <div className="flex gap-4 justify-end">
        ...
        <div className="flex justify-between items-center p-0.5">
          <h3 className="flex items-center gap-1 text-[13px] whitespace-nowrap text-neutral-500 tracking-wider">
            <SquarePen className="w-4" />{rule.created_by} {convertDate(rule.created_at)}
          </h3>
        </div>

        <div className="flex justify-between items-center p-0.5">
          <h3 className="flex items-center gap-1 text-[13px] whitespace-nowrap text-neutral-500 tracking-wider">
            <Repeat className="w-4" />{rule.updated_by} {convertDate(rule.updated_at)}
          </h3>
        </div>

        <div className="flex justify-between items-center p-0.5">
          <h3 className="flex items-center gap-1 text-[13px] whitespace-nowrap text-neutral-500 tracking-wider">
            <Smile className="w-4" />{readUsers.length} / {users.length}
          </h3>
        </div>
      </div>
