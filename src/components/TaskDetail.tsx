...
export default function TaskDetail({ task, user, unreadIds, onClose, onEdit }: TaskDetailProps) {
  ...
  const statusStyles: Record<string, string> = {
    "未着手": "bg-neutral-200 text-neutral-800",
      ...
      >
        ...
        <li className="col-span-2 flex gap-4">
          ...
          <div className="flex flex-col w-[calc((100%-3rem)/4)]">
            ...
            <p className={`py-1 px-2 rounded-md text-center ${statusStyle}`}>{task.status}</p>
          </div>
          <div className="flex flex-col w-[calc((100%-3rem)/4)]">
            ...
            <p className={`py-1 px-2 rounded-md ${priorityStyle !== "" ? priorityStyle + " text-center" : "bg-neutral-200"}`}>{task.priority ? task.priority : "-"}</p>
          </div>
        </li>
        ...
        <li className="flex flex-col col-span-2 pb-1">
          ...
          {task.remarks ? (
            <div className={`whitespace-pre-wrap tiptap-base tiptap-viewer bg-neutral-200 py-1 px-2 rounded-md`} dangerouslySetInnerHTML={{ __html: tiptapMarkdownToHtml(task.remarks) }} />
          ) : (
